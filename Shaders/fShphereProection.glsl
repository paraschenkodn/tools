...
void main(void) {
    ...
    if (true) {   // заполняем цветом
        gl_FragColor.x=IDf.x; // запоминаем ИД модели объекта
        gl_FragColor.y=IDf.y; // запоминаем ИД fragmenta
        gl_FragColor.z=10;  // запоминаем глубину выбранного фрагмента, для последующей селекции (?? как прочитать в шейдере)
        gl_FragColor.w=10;    // ??? а если цвет заливки такой
    }
    // *****
}
