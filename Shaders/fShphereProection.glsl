...
void main(void) {
    ...
    if (selectmode) {   // заполняем цветом
        gl_FragColor.x=0.0;//IDf.x; // запоминаем ИД модели объекта
        gl_FragColor.y=IDf.y; // запоминаем ИД fragmenta
        gl_FragColor.z=0.5;  // запоминаем глубину выбранного фрагмента, для последующей селекции (?? как прочитать в шейдере)
